<?php
return array(
    //home page
    'home'=>'主頁',
    'myshoppinglist'=>'我的购物車',
    'myaccount'=>'我的帳戶',
    'myfavorite'=>'我的收藏',
    'newseries'=>'新系列',
    'brand'=>'品牌',
    'category'=>'类型',
    'baby'=>'嬰兒',
    'girl'=>'女孩',
    'boy'=>'男孩',
    'accessory'=>'配飾',
    'shoes'=>'童鞋',
    'sales'=>'特卖场',
    'featured'=>'精選',
    'clothingmatch'=>'穿衣搭配',
    'search'=>'搜索',
    'bytime'=>'按时间',
    'lastsevenday'=>'最近7天',
    'lastfourweek'=>'最近4个星期',
    'commingsoon'=>'即将有货',
    'addToShoppingList'=>'添加到購物車',
    'addToFavoriteList'=>'添加到我的收藏',
    'recentShoppingList'=>'最近加入的宝贝',
    'totalAmount'=>'总金额',
    'buynow'=>'立即購買',
    'seeall'=>'查看全部',
    'forgetpwd'=>'忘記密碼',
    'loginAccount'=>'登陸我的賬戶',
    'login'=>'登陸',
    'rmbme'=>'記住我',
    'china'=>'中國',
	'hk'=>'香港',
	'or'=>'或',
	'createaccount'=>'創建我的帳戶',
	'signupnow'=>'立即註冊',
	'subscribe'=>'我希望訂閱starball新聞咨詢和優惠信息。',
	'simpleC'=>'簡體中文',
	'traditionalC'=>'繁體中文',
	'newcuscre'=>'新客戶創建一個賬戶',
	'myorder'=>'我的訂單',
	'return'=>'退貨',
	'mycoupon'=>'我的優惠券',
	'mygiftcard'=>'我的禮品卡',
	'signout'=>'退出',
	'Cmyfavorites'=>'查看我的收藏',
	'totalamt'=>'總金額',
	'favoritempty'=>'您的收藏夾爲空',
	'Callfeatures'=>'查看全部精選',
	'createAacct'=>'創建一個賬戶',
	'ubagempty'=>'您的購物袋為空',
	'size'=>'尺碼',
	'quantity'=>'數量',
	'Cmybag'=>'查看我的購物車',
	'emptybag'=>'購物袋還是空的！',
	'nothing'=>'您的購物車沒有任何商品',
	'nothinginfavorite'=>'您的收藏夾裏沒有任何商品',
	'continueshopping'=>'繼續購物',
	'productlist'=>'商品列表',
	'giftpackage'=>'禮品包裝與禮品卡信息',
	'shoppingcart'=>'購物車',
	'confirm'=>'確認',
    'buyrecord'=>'我的購買紀錄',
    'shippingfee'=>'運費',
    'packagefee'=>'禮物包裝費用',
    'ordertotal'=>'我的訂單總額',
    'totalpay'=>'支付總額',
    'submitorder'=>'提交定單',
    'tax'=>'關稅',
	'noextra'=>'當您收取貨物時，沒有任何額外費用！',
    'needhelp'=>'需要幫助嗎？',
    'Cshiptime'=>'查看我們的配送時間',
    'CRpolicy'=>'查看我們的退或政策',	
    'create'=>'創建',
    'myaddress'=>'我的地址',
    'male'=>'男',
	'female'=>'女',
	'consignee'=>'收貨人姓名',
    'address'=>'地址',
    'zipcode'=>'郵政編碼',
    'citytown'=>'城鎮/城市',
    'country'=>'國家/地區',
    'moredetail'=>'更多细节',
    'addressHkDetail'=>'如選擇地鐵站交收，請您留下聯繫電話，我們會有專人和您聯絡商討送貨事宜。',
    'submit'=>'提交',
    'setaddresscheck'=>'請為此地址設定一個名稱，以便日後查詢',
    'useremail'=>'用戶名/電子郵件',
    'pwd'=>'密碼',
    'shippingaddress'=>'收貨地址',
    'ADDaddress'=>'添加地址',
    'email'=>'電子郵件',
    'confirmpwd'=>'確認您的密碼',
    'nickname'=>'昵稱',
    'contactnumber'=>'聯絡號碼',
    'color'=>'顏色',
    'details'=>'詳細信息',
    'ingredient'=>'成份',
    'shipreturn'=>'退貨與送貨',
    'contactus'=>'聯繫我們',
    'filter'=>'篩選',
    'sex'=>'性別',
	'age'=>'年齡',
	'season'=>'季節',
	'class'=>'類別',
	'discount'=>'折扣',
	'all'=>'全部',
	'paymode'=>'支付方式',
	'shippayinfo'=>'配送和支付信息',
	'wechat'=>'微信',
	'alipay'=>'支付寶',
	'payok'=>'支付成功',
	'payfail'=>'支付失敗',
	'backhome'=>'返回首頁',
	'viewOrderDetail'=>'查看訂單詳情',
	'dearcus'=>'親愛的用戶',
	'oksuccess'=>'您的訂單已支付成功，訂單號為',
	'ordernum'=>'訂單號',
	'payable'=>'應付金額',
	'testpaysuccess'=>'測試成功支付',
	'shipping'=>'送貨',
	'pay'=>'付款',
	'followus'=>'關注我們',
	'brandauth'=>'品牌授權',
	'certify'=>'100%正品保證',
	'cusser'=>'顧客服務',
	'callemail'=>'可以致電或電郵聯絡',
	'dayship'=>'付款秒發',
	'SFYT'=>'原裝進口，速運到家',
	'srcontent'=>'退貨與送貨：

				  送貨：
				  下單之後保證24小時內發貨，因為商品均屬於國外進口品牌，所以中國大陸地區默認順豐速運，香港地區****。
					
				  退貨：
				  若商品有質量（破洞等）問題，請在簽收商品後7日之致電或發送郵件聯繫客服。
					
				  若涉及到商品尺碼不合適相關問題，在保留吊牌及外包裝且商品保存完整的條件下，可以進行尺碼更換，需買家承擔往返運費，恕不接受運費到付。
				  若無更換的尺碼，恕不接受退換貨，為保證每位寶寶的使用安全和衛生，請務必確認購買的款式和尺碼。
					
				  若商品無質量問題，因個人喜好（款式，顏色，外觀等）要求退換貨恕不接受退換，敬請諒解，謝謝配合。',
	'CScontent'=>'球姐告诉我，我再告诉你',
	
	'PaySec'=>'交易安全',
	'PayCer'=>'支付全程保證',
	'shipinfo'=>'物流配送',
	'addressName'=>'地址名稱',
	'RtrnPolicy'=>'退換政策',
	'SizeGuide'=>'尺碼手冊',
	'FAQ'=>'常見問題',
	'deliveryToHome'=>'送货上门(35 HKD)',
	'pickAtStation'=>'地鐵站交接(免運費)',
	'comingsoonalert'=>'(即將到貨 - 設置提醒)',
	'soldout'=>'售罄',
	'sizecomingsoon'=>'此尺碼即將到貨',
	'notifymewhengoodarrived'=>'此商品到貨時提醒我',
	'AboutUs'=>'關於我們',
	'CcontactUS'=>'StarBall.Kids非常歡迎來自顧客朋友的反饋以及合作夥伴的諮詢。請發送郵件給我們，我們會盡快和您聯繫。<br><br>
 

					有關於品牌合作的事宜：<br><br>
					請發送郵件至 info@starballkids.com<br><br>
					有關於商品信息查詢或反饋：<br><br>
					大陸地區請聯繫微信客服：starballkidshk<br>
					香港地區請聯繫whatsapp客服： 852 59697187<br>
					或發送郵件至service@starballkids.com<br>',
					
	'Cpaymode'=>'目前StarBall.Kids官方網站支持 支付寶 和 微信支付 兩種付款渠道（以人民幣結算）。<br>
			     香港地區也可以whatsapp聯繫客服轉帳匯款，或於送貨時以現金支付。',
     'Cshipinfo'=>' 大陸地區：<br>默認 順豐快遞公司
					商品郵費按照順豐公司收費標準收取；如單筆交易消費滿人民幣990元以上的商品，包郵。<br><br>
					
					香港地區：<br>支持兩種交收貨方式<br>
					選擇地鐵站交收：不收運費 （注：面交僅限於地鐵站）<br>
					選擇送貨上門：運費35港幣；單筆交易消費滿港幣1190以上免運費。',
					
	'CRtrnPolicy'=>'若商品有問題（比如破洞，開線，污漬等），請在簽收商品後7日之內聯繫微信客服(starballkidshk)或whatsapp(85259697187)客服或發送郵件(service@starballkids.com)溝通相關退貨事宜。<br>

					退貨的具體條件如下：<br>
					1. 商品的外包裝完整無損，吊牌及相關配件（若有）齊全。<br>
					2. 除了商品本身的質量問題，商品本身無明顯穿著痕跡，無洗滌，劃痕或破損等痕跡。<br>
					3. 退款金額為客戶實際支付的貨款。<br>
					4. 內衣，內褲以及褲襪等貼身衣物因衛生問題不支持退貨服務。<br>
					5. 退貨商品往返運費均由賣家承擔。<br>
					若商品有質量問題，除了選擇退貨以外，客戶還可選擇一定的金額賠償，具體額度要看商品本身的質量問題程度，詳情請與客服溝通。<br><br>
					
					換貨的具體條件如下：<br>
					
					1. 商品的外包裝完整無損，吊牌及相關配件（若有）齊全。<br>
					2. 商品本身無明顯穿著痕跡，無洗滌，劃痕或破損等痕跡。<br>
					3. 若是尺碼不合適的問題，可以進行同款尺碼調換。需買家承擔往返運費，恕不接受運費到付。若無可調換的尺碼，只可接受等價換貨，不接受退貨，買家需承擔往返運費。<br>
					4. 若商品無質量問題，因個人喜好（款式，顏色，外觀等）要求退換，恕不能夠接受，敬請諒解，謝謝配合。',
	'Csizeguide'=>'請注意官網列出的商品尺碼均為通用的尺碼，但是每個品牌也有自己的詳細尺碼列表。<br>

					建議在選購商品時，請結合通用尺碼和品牌自己的尺碼信息來選購。<br>有關於品牌自己的尺碼信息，請點擊每個商品圖片右邊的尺碼二字。'	,
    'CFAQ'=>'怎樣在StarBall.Kids官網購買產品？<br>

				在下單購買之前，首先您要在官網註冊一個個人帳戶，點擊官網右上角 我的帳戶－新客戶創建一個帳戶 即可。註冊完帳戶，就可以下單購買了。<br><br>
				
				 
				
				忘記用戶名或密碼而未能登入？<br>
				
				找回密碼或用戶名請聯繫微信客服或whatsapp客服或發郵件均可。<br><br>
				
				 
				
				收到商品時候需要支付關稅麼？<br>
				
				目前在官網購買的商品不需要支付任何的關稅。<br><br>
				
				 
				
				怎樣知道商品有無貨或何時補貨？<br>
				
				只要商品可以加入到購物車就表明該商品有庫存。關於補貨事宜，因品牌而異。<br><br>
				
				 
				
				怎樣了解關於StarBall.Kids的商品推廣和更新以及一些客戶福利和抽獎？<br>
				
				請關注我們的社交媒體帳號，所有帳號同步更新：<br>
				
				微博weibo: StarBallKids<br>
				
				facebook: StarBall.Kids<br>
				
				Instagram: starballkids<br>
				
				微信公眾號：starballkids',
	'CaboutUs'=>'StarBall.Kids是一間成立於香港的網上嬰幼兒童進口設計師品牌集合店，從產品質量，安全，舒適度和設計風格四大考慮出發，在全球範圍內搜羅最好的兒童服裝品牌。
 				<br><br>

				StarBall.Kids的创始人Patrick和Catherine是一對生活在香港的夫婦，作為家長，他們發現市面上的幼童品牌雖然數量不少，但很少能同時兼顧到產品質量，安全，舒適度和設計風格這四大因素。StarBall.Kids留意到很多優質的歐美童裝並沒有進入亞洲市場，而對於這些時尚又特別的優質進口品牌，媽媽們只可以選擇海外代購，價格高又費時，更遑論服務和產品的保證。而現在，StarBall.Kids與這些優秀的品牌合作，建立這樣一個平台，就是希望能夠給父母更多的品牌選擇，提供更方便，更暢快，更實惠的購物體驗。

                <br><br>

				從StarBall.Kids創立至今，“Quality，Comfort and Cuteness”是我們一直堅持的，也是我們合作品牌的特質，優質又兼顧時尚，舒適又不失可愛。兒童服裝品牌繁多，我們會致力挑選和搜羅最優質的帶給各位爸爸媽媽，陪伴您孩子的成長。',
	'HKDPaymentNotOpen'=>'目前港幣支付渠道尚未開通，請切換成人民幣進行支付。若仍希望通過港幣支付，請直接聯系賣家。',
	'payInProgress'=>'正在支付...',
	'finishPayment'=>'已完成支付',
	'paymentProblem'=>'支付遇到問題',
	'inputGreetingWords'=>'StarBall.Kids爲您免費提供寫祝福語的功能。如果您購買StarBall.Kids商品送作禮物，可以在此處寫下對寶寶祝福的話語（不超過250個字），我們會有專門的卡片包含在商品裏面，一起送到您的手中。',
	'footsizeunit'=>'碼',
	'shoeSize'=>'鞋碼',
	'mobile'=>'電話',
	'personalInfoChanged'=>'個人信息修改成功',
	'socialMediaLoginUsrMsg1'=>'您通過社交媒體登錄,可以設置新帳號或者綁定現有帳號,方便您在其它浏覽器上登錄。您也可以',
	'socialMediaLoginUsrMsg2'=>'跳過此步',
	'socialMediaLoginUsrMsg3'=>',稍後再做設置。',
	'passwordChanged'=>'密碼修改成功',
	'existingPwdIncorrect'=>'現有密碼不正確',
	'newPwdSameWithExisting'=>'新密碼不能和現有密碼相同',
	'newPwdDifferentWithRepeat'=>'新密碼和確認重複密碼不一致',
	'newPwdShouldNotBeEmpty'=>'新密碼不能爲空',
	'setupNewPwd'=>'設置新密碼',
	'changePwd'=>'更改密碼',
	'newPwdFormatError'=>'新密碼格式不正確,必須爲6位到22位數字和字母的組合,支持@*#,請重新輸入',
    'userNameEmpty'=>'用戶名不能爲空',
    'emailEmpty'=>'郵箱不能爲空',
    'emailFormatError'=>'郵箱格式錯誤',
    'mobileShouldbeNumber'=>'電話號碼必須爲數字',
    'maintenance'=>'手機端網站即將上線,請您先通過PC端進行訪問,謝謝支持。',
    'addressmsg1'=>'您可以在提交訂單時添加多個送貨地址（家庭住址和工作地址等）。請標注不同的名稱，以便在您下訂單時快速查找。',
    'addressmsg2'=>'您可以在下面刪除您所添加的地址。',
    'personalinfo'=>'我的個人信息',
    'personalinfoChange'=>'如果您的個人信息發生改變，請隨時進行更新。',
    'pickitem'=>'選貨',
    'emailNotExist'=>'郵箱不存在',
    'newUserSetupSuccess'=>'新用戶設置成功',
    'bindUserSuccess'=>'用戶綁定成功'
);